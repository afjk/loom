...
**パラメータ：**
- `predicate`（必須、文字列）：簡易な条件式。第一段階では JS 式ではなく、限定された記法（例：`"value > 0"`、`"key == 'Enter'"`）のみ受け付ける。具体的記法は第一段階プロトタイプで確定する。

**説明：**

入力イベントのうち predicate を満たすものだけを通過させる。
...
複数の Event ストリームを1本にまとめる。同一フレームに両方発生した場合、両方を順番に下流へ流す（配列として受け取れる）。
