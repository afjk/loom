...
**出力：**
- `event`（クリックイベント）
  - 型：`event<vec2>`（ペイロードはクリック位置 `{x, y}`、ビューポート座標、ピクセル単位）
...
**パラメータ：**
- `predicate`（必須、文字列）：簡易な条件式。第一段階では JS 式ではなく、限定された記法（例：`"value > 0"`、`"key == 'Enter'"`）のみ受け付ける。実装は `new Function("value", "key", "return (" + predicate + ");")` で関数化する。ペイロード値が `value`、文字列の場合は `key` に渡される（未指定の場合は `undefined`）。

**説明：**

入力イベントのうち predicate を満たすものだけを通過させる。predicate の関数化に失敗した場合は `INVALID_GRAPH` エラーを投げる。
