...
入力エッジが接続されていないポートの値は `undefined` です。各ノード型は、以下のようにこれを処理します：

- `add`、`multiply`：未接続の `b` ポートは、`params.b` のデフォルト値で処理
- `sine`：入力 `t` が必須。未接続の場合の動作は未定義（通常はエラー）
...
- 状態の初期化・リセット戦略の確立
- マルチプレイ環境での状態同期ポリシー（CRDT など）の検討
...
### 第五段階：マルチプレイ・broadcast・アダプタ

- マルチプレイ環境での複数クライアント同期
- broadcast ノードによるイベント配信
- Three.js / SceneSync 連携アダプタ
- AI 連携ツール定義（Claude の Function Calling との統合など）
