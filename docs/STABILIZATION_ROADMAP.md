...
4. Stabilize node definition schema
   ...
   - Document runtime registry in `docs/RUNTIME_NODE_REGISTRY.md`
   - Add runtime registry shape tests in `test/runtime-node-registry.test.mjs`
   - Add metadata/runtime drift detection in `test/runtime-metadata-drift.test.mjs`
...
4. Add metadata schema tests to verify node definition shape.
   ...
   - Runtime registry shape is covered by `test/runtime-node-registry.test.mjs`.
   - Metadata/runtime drift is covered by `test/runtime-metadata-drift.test.mjs`.
5. Evaluate whether to introduce a formal runtime registration API after registry/metadata drift tests are stable.
